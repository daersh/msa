...
public class OrderController {
    ...
    public ResponseEntity<List<ResponseOrder>> getUserOrders(@PathVariable String userId){
        List<OrderDTO> orderDTOList = orderService.getUserOrders(userId);
        orderDTOList.stream().forEach(System.out::println);
        List<ResponseOrder> returnValue = orderDTToResponseOrder(orderDTOList);
        return ResponseEntity.status(HttpStatus.OK).body(returnValue);
    }

    private List<ResponseOrder> orderDTToResponseOrder(List<OrderDTO> orderDTOList) {
